...
class iCNN_Node(torch.nn.Module):
    ...
    def forward(self, x, feature_from_pre, feature_from_post):
        ...
        x_out = self.dropout(torch.tanh(self.inter_conv_node_batchnorm(self.inter_conv_node(input=x_out))))
        y = self.dropout(torch.tanh(self.conv_node_batchnorm(self.conv_node(x_out))))
        return y
...
class FaceModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        inputs = [self.dropout(F.relu(self.input_bnm[0](self.input_conv[0](scaled_x[0])), inplace=True))]
        for i in range(1, self.recurrent_number):
            inputs.append(self.dropout(F.relu(self.input_bnm[i](self.input_conv[i](scaled_x[i])), inplace=True)))
        ...
        final_output = self.dropout(torch.tanh(self.last_con1_bnm(self.last_conv1(output))))
